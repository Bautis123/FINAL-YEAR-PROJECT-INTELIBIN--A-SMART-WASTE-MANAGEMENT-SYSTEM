...
<?php endif; ?>
<?php foreach ($ib_rows as $ib_r): $ib_st = ib_status($ib_r['p'], (int)$vm['bin']['alert_at']); $ib_d = $ib_r['d'] ?? ib_dist($ib_r['p'], $vm['bin']); ?>
        <tr data-ib-s="<?= $ib_st['key'] ?>"><td><?= ib_time($ib_r['t'], true) ?></td><td><?= number_format($ib_d, 1) ?> cm</td><td><span class="ib-bar"><span style="width:<?= number_format($ib_r['p'], 0) ?>%"></span></span><?= number_format($ib_r['p'], 0) ?>%</td><td><span class="ib-pill ib-sm"><i></i><?= $ib_st['label'] ?></span></td></tr>
<?php endforeach; ?>
      </tbody>
    </table>
  </div>
